...
function InsightWs(serverUrl, network, routePrefix) {
    ...
    if (Networks.get(serverUrl)) {
        ...
        if (network === Networks.livenet) {
            serverUrl = 'https://insight.bitpay.com';
        } else if (network === Networks.testnet) {
            serverUrl = 'https://test-insight.bitpay.com';
        }
    }

    var url = serverUrl ?
        serverUrl.replace(endingSlash, "") + '/'
        + prefix : (Networks.defaultNetwork === Networks.livenet) ? 'https://insight.bitpay.com'
            + '/' + prefix : 'https://test-insight.bitpay.com' + '/' + prefix;
    Insight.apply(insightWs, [url, network]);

    var wsUrl;
    if (serverUrl) {
        wsUrl = serverUrl.replace(endingSlash, "") + '/';
    } else {
