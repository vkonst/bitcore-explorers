...
function InsightWs(serverUrl, network, routePrefix) {
    ...
    var url = '';
    routePrefix = routePrefix ? '/' + routePrefix : '/api/';

    if (Networks.get(serverUrl)) {
        network = Networks.get(serverUrl);
        if (network === Networks.livenet) {
            serverUrl = 'https://insight.bitpay.com/api/';
        } else {
            serverUrl = 'https://test-insight.bitpay.com/api/';
        }

        url = serverUrl ?
            serverUrl.replace(/\/$/, "") : Networks.defaultNetwork;
        console.log(url);
    } else {
        url = serverUrl ?
            serverUrl.replace(/\/$/, "") + routePrefix : Networks.defaultNetwork;
        console.log(url);
    }

    // var url = serverUrl ?
    //     serverUrl.replace(/\/$/, "") + API_PREFIX : Networks.defaultNetwork;
    network = network ? network : Networks.defaultNetwork;

    Insight.apply(insightWs, [url, network]);

    if (serverUrl) {
        JSUtil.defineImmutable(insightWs, {
            wsUrl: serverUrl.match(/.?\/$/) ? serverUrl : serverUrl + '/',
            network: network
        })
    } else {
        JSUtil.defineImmutable(insightWs, {
    ...
    insightWs.getBlock(blockHash, function(err, block) {
        if (err) {
            ...
            if (blockHash === block.hash) {
                ...
                insightWs.events.emit('insightWs:error', new Error("Get request return incorrect block"));
            }
        }
    });
};
